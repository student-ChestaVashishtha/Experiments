...
logfilepath=os.path.join(logdir,'data_preprocessing.log')
...
def transformed_text(text: str)-> str:
    ...
    text=[word for word in text if word not in  stopwords.words('english') and string.punctuation]
...
def preprocess_data(df,text_column='text',target_column='target'):
    try:
        ...
        df.loc[:, text_column] = df[text_column].apply(transformed_text)
